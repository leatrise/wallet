...
settings_preferences_default_leverage = 默认杠杆
...
info_funding_payments_title = 资金费用
info_funding_payments_description = 资金费用是交易者之间的定期支付，用于使永续合约价格接近标的资产现货价格。正资金费用表示多头支付空头，负资金费用表示空头支付多头。
info_funding_rate_title = 资金费率
info_funding_rate_description = 资金费率决定持有永续仓位的成本。它按小时计算，有助于维持永续合约与标的资产现货价格之间的价格平衡。
info_liquidation_price_title = 强平价格
info_liquidation_price_description = 强平价格是当市场价格达到该水平时，您的仓位将被自动平仓以防止进一步亏损的价格。当市场价触及此价位，仓位被强制平仓，保证金将被扣除。
info_open_interest_title = 未平仓合约
info_open_interest_description = 未平仓合约代表所有尚未结算的永续合约的总价值，反映市场活动和流动性。
