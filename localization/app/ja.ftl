...
settings_preferences_default_leverage = デフォルトレバレッジ
...
info_funding_payments_title = ファンディングペイメント
info_funding_payments_description = ファンディングペイメントは、永久契約の価格を基礎資産のスポット価格に近づけるためにトレーダー間で定期的に行われる支払いです。プラスのファンディングではロングポジションがショートポジションに支払い、マイナスのファンディングではショートポジションがロングポジションに支払います。
info_funding_rate_title = 資金調達率
info_funding_rate_description = ファンディングレートは永久ポジションの保有コストを決定します。これは毎時計算され、永久契約と基礎資産のスポット価格の価格均衡を維持するのに役立ちます。
info_liquidation_price_title = 清算価格
info_liquidation_price_description = 清算価格は、損失拡大を防ぐためにポジションが自動的に決済される価格水準です。市場価格がこの水準に達するとポジションは清算され、証拠金を失います。
info_open_interest_title = 未決済建玉
info_open_interest_description = オープンインタレストは、未決済のすべての永久契約の合計価値を表します。市場の活動状況や流動性の指標となります。
